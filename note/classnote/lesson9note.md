...
> 一种比较典型的分法：  
> ![](http://7xotr7.com1.z0.glb.clouddn.com/16-5-13/55690970.jpg)

> 评估分法：  
> ![](http://7xotr7.com1.z0.glb.clouddn.com/16-5-13/24853303.jpg)  

由于正样本非常少，所以是个偏斜率问题：  
> ![](http://7xotr7.com1.z0.glb.clouddn.com/16-5-13/67749454.jpg)  
> 那么用什么评价度量好呢？ 因为数据是非常偏斜的 因为y=0是 更加常见的 因此分类准确度不是一个好的度量法 (比较偏斜的数据集 那么总是预测y=0 它的分类准确度自然会很高 )。 取而代之的  我们应该算出 真阳性、假阳性、 假阴性和真阴性的比率 来作为评价度量值 我们也可以算出查准率和召回率 或者算出 F1-积分 通过一个很简单的数字 来总结出查准和召回的大小 通过这些方法 你就可以评价你的异常检测算法 在交叉验证和测试集样本中的表现 

关于ε：

> ε是我们用来决定 什么时候把一个样本当作是 异常样本的一个阈值。   
> 如果你有 一组交叉验证集样本，一种选择参数ε的方法 就是你可以试一试 多个不同的 ε的取值 然后选出一个 **使得F1-积分的值最大的那个ε** 也就是在交叉验证集中表现最好的 


#### 1.2.2 Anomaly Detection vs. Supervised Learning  
8 分

主要问题：什么时候使用异常检测算法？什么时候用监督学习？这两种算法到底有什么区别？  

> ![](http://7xotr7.com1.z0.glb.clouddn.com/16-5-13/3117320.jpg)  
> ![](http://7xotr7.com1.z0.glb.clouddn.com/16-5-13/27144815.jpg)
...
> 另外可能会遇到的一些问题 通常来说 正样本的数量很少 甚至有时候是0 也就是说 出现了太多没见过的 不同的异常类型 那么对于这些问题 通常应该使用的算法就是 异常检测算法

练习：  

> ![](http://7xotr7.com1.z0.glb.clouddn.com/16-5-13/10166782.jpg)

#### 1.2.3 Choosing What Features to Use  
12 分

主要问题： 如何选择异常检测算法的特征变量 
